...
namespace GunlibMagic
{
    internal class Gunlib
    {
        ...
        {
            GunlibMagic.Gunlib.StartBothGuns(() =>
            {
                if (spherepointer == null)
                {
                    spherepointer = GameObject.CreatePrimitive(PrimitiveType.Cube);
        ...
        private static void UpdateTrail(Vector3 start, Vector3 end)
        {
            ...
            float t = Time.time;
            Vector3 targetBend =
                perpA * (Mathf.Sin(t * 1.1f) * BendStrength * dist * 0.18f) +
                perpB * (Mathf.Sin(t * 0.7f + 1.3f) * BendStrength * dist * 0.14f);
